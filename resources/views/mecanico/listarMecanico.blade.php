    ...
    <div class="containerHomeAll">
        <div class="containerInfoAll">
            <div class="containerTarefas">
                <h1 class="tarefas">Aqui estão todos mecânicos cadastrados</h1>
                @if (session('msg'))
                    <div class="alertLista" style="display: block;">
                        <div class="alert alert-success alert-dismissible fade show" role="alert">
                            {{ session('msg') }}
                            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"
                                onclick="clearMsg()"></button>
                        </div>
                    </div>
                @endif
                <script>
            ...
            <div class="containerServicos">
                @if ($mecanicos->isEmpty())
                    <div class="containerTarefas">
                        <h1 class="tarefas">Não há mecanicos cadastrados</h1>
                    </div>
                @endif
                @foreach ($mecanicos as $mecanico)
                    <div class="containerServicoInfo">
                        <div class="containerInfo">
                            <h1 class="infoServico">Nome: <p class="info">{{ $mecanico->nome }}</p>
                            </h1>
                        </div>
                        <div class="containerInfo">
                            <h1 class="infoServico">Especialidade: <p class="info">{{ $mecanico->especialidade }}</p>
                            </h1>
                        </div>
                        <div class="containerInfo">
                            <h1 class="infoServico">E-mail: <p class="info">{{ $mecanico->email }}</p>
                            </h1>
                        </div>
